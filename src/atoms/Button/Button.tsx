...
/**
 * Usamos HTMLMotionProps<"button"> para mezclar correctamente
 * los props del botón nativo + los de Motion (animate, whileTap, etc).
 * Omitimos "size" para no chocar con nuestra prop `size`.
 */
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const safeVariant: ButtonVariant =
      variant in variantStyles ? variant : "primary";
    ...
    const safeSize: ButtonSize = size in sizeStyles ? size : "md";
    ...
    const buttonContent = (
      <>
        ...
        {!loading && LeftIcon && <LeftIcon className={iconSize} aria-hidden />}

        {safeVariant !== "icon-only" && (
          <span className={loading ? "opacity-0" : ""}>{children}</span>
        )}

        {!loading && RightIcon && (
          <RightIcon className={iconSize} aria-hidden />
        )}
      </>
    );
    ...
    return (
      <motion.button
        ...
        aria-label={
          ariaLabel ||
          (safeVariant === "icon-only" ? String(children) : undefined)
        }
